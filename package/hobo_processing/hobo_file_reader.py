...
class HoboDataContainer():
    ...
    # trim_data and all of its helper methods assume that self.dataframe is
    # populated with a valid pandas dataframe
    def trim_data(self):
    ...
    def _clean_column_names(self):
        renamed_columns = {c: c[:c.find(
            "(") - 1] if "(" in c else c for c in self.dataframe.columns}
    ...
    def _remove_dropable_columns(self):
        ...
        self.dataframe.drop(labels=dropable_columns, axis=1,
                            inplace=True, errors='ignore')

    # remove columns that only contain NaN values
    def _remove_nan_columns(self):
        nan_columns = [x for x in list(self.dataframe.columns) if self.dataframe[
            x].isnull().all()]
    ...
    def _remove_logging_information(self):
        ...
        if(len(null_indices) >= 2):
            ...
            differences = [null_indices[i] - null_indices[i - 1]
                           for i in range(1, len(null_indices))]
            max_index = max([i for i in range(0, len(differences))],
                            key=lambda x: differences[x])

            keep_range = range(
                null_indices[max_index] + 1, null_indices[max_index + 1])

            drop_indices = [x for x in range(
                0, len(index)) if x not in keep_range]
            self.dataframe.drop(self.dataframe.index[
                                drop_indices], inplace=True)
        elif(len(null_indices) == 1):
            self.dataframe.drop(self.dataframe.index[
                                null_indices], inplace=True)

    # assumes that self.dataframe is already populated with a valid pandas
    # dataframe
    def verify_valid_datafile(self):
        # Checks to make sure all index entries are Timestamp objects
        check_timestamps = lambda df: all(
            type(x) == pd.tslib.Timestamp for x in list(df.index))
        # Checks to make sure at least some of the valid column names are
        # represented
        check_legal_column_names = lambda df: True
        self.valid_datafile = check_timestamps(
            self.dataframe) and check_legal_column_names(self.dataframe)
    ...
    def infer_sensor_type(self):
        ...
        num_matching_columns = lambda sensor_type: sum(
            1 for col_name in columns if col_name in self.legal_columns[sensor_type])
        normalized_matching_columns = lambda sensor_type: float(
            num_matching_columns(sensor_type)) / len(self.legal_columns[sensor_type])

        scores = {sensor_type: normalized_matching_columns(
            sensor_type) for sensor_type in self.legal_columns.keys()}
    ...
    def infer_boolean_data(self):
        ...
        is_boolean_column = lambda col_name: all(
            x in boolean_values for x in self.dataframe[col_name].dropna())
    ...
    def infer_even_time_increments(self):
        ...
        if(len(index) > 1):
            ...
            self.even_time_increments = all(
                index[i] - index[i - 1] == first_diff for i in range(2, len(index)))
        else:
            self.even_time_increments = False
    ...
    # If either start_time or end_time are None, the start and end times of
    # the entire dataframe are used
    ...
    def series_time_percentage(self, series_name):
        ...
        time_differences = time_stamps.diff(periods=1).shift(
            periods=-1).fillna(pd.Timedelta('0 days'))
    ...
    def calculate_interval_statistic(self, series_name, interval_length, resampler, start_time=None, end_time=None):
        closest_start, closest_end = self.get_closest_start_end_times(
            start_time, end_time)
