...
class Plotter():
    ...
    default_colors = colors = [QColor(Qt.blue), QColor(Qt.green), QColor(Qt.yellow), QColor(Qt.red)]
...
class Light_Occupancy_Pie_Chart_Plotter(Plotter):
    ...
    def get_default_parameters(self):
        title = self.title_gen(self.subset_index)
        return param_utils.Parameter_Collection(OrderedDict([(title_param, title), (self.color_param, self.default_colors)]))

    def plotting_function(self, figure, hdc):
        axes = self.get_axes(figure)
        subset_function = list(self.subset_functions.keys())[self.subset_index]
        title = self.parameters[title_param]
        patches, texts = self.subset_pie_chart(axes, hdc, title, self.graph_labels, subset_func=subset_function)
...
class State_Bar_Chart_Plotter(Plotter, gen_plt.Generic_Bar_Plotter):
    ...
    def get_default_parameters(self):
        ...
        return param_utils.Parameter_Collection(defaults)
    ...
    def plotting_function(self, figure, hdc):
        axes = self.get_axes(figure)
        data = self.get_data(hdc)
        kwargs = dict(title=self.parameters[title_param], x_ticks=("Open", "Closed"), x_label=self.parameters[x_label_param],
                      y_label=self.parameters[y_label_param], color=self.parameters[color_param].name())
...
class Single_Bar_Subinterval_Plotter(Plotter, gen_plt.Generic_Bar_Plotter):
    ...
    def get_default_parameters(self):
        ...
        y_label = "Average Value: %s" % self.column_name
        defaults = OrderedDict([(title_param, title),
                                (x_label_param, "Time Period"),
                                (y_label_param, y_label),
                                (color_param, QColor(0, 0, 255))])
        return param_utils.Parameter_Collection(defaults)
